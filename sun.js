...
// ============================================================
// ✅ PHẦN THÊM MỚI HOÀN CHỈNH THEO YÊU CẦU /lc79‑md5
// ============================================================
// Tạo chuỗi pattern dạng t/x ngắn hạn (vd: xxtxtxxxtxtxxxtxtx)
function buildPatternString(data, len = 18) {
  const arr = data.slice(0, Math.min(len, data.length)).map(d => 
    (d.Ket_qua || d.ket_qua || '').toLowerCase().includes('tài') ? 't' : 'x'
  );
  return arr.join('') || 'xxxxxxxxxx';
}
// Lấy tên loại cầu chính nhất
function getMainCauName(allPatterns = []) {
  if (!allPatterns || allPatterns.length === 0) return 'Cầu Tự Nhiên';
  const sorted = [...allPatterns].sort((a, b) => (b.priority || 0) - (a.priority || 0) || (b.confidence || 0) - (a.confidence || 0));
  return sorted[0].name || 'Cầu Tự Nhiên';
}
...
// ✅ ROUTE MỚI ĐÚNG YÊU CẦU: /lc79‑md5
app.get('/lc79-md5', async (req, res) => {
  try {
    res.setHeader('Content‑Type', 'text/plain; charset=utf‑8');
    const data = await fetchData();
    if (!data || !data.data || data.data.length === 0) {
      return res.send('@cskhvilong1 Lỗi không lấy được dữ liệu từ API');
    }

    setTimeout(() => verifyPredictions('b52', data.data).catch(() => {}), 0);

    const prev = data.data[0];
    const phien_truoc = prev.Phien;
    const x1 = prev.xuc_xac_1 ?? 0;
    const x2 = prev.xuc_xac_2 ?? 0;
    const x3 = prev.xuc_xac_3 ?? 0;
    const tong = prev.tong ?? (x1 + x2 + x3);
    const ket_qua = prev.Ket_qua || (tong >= 11 ? 'Tài' : 'Xỉu');
    const phien_hien_tai = Number(phien_truoc) + 1;

    const result = calculateAdvancedPrediction(data.data, 'b52');
    const du_doan = result.prediction;
    const ti_le = result.confidence;
    const pattern_str = buildPatternString(data.data, 18);
    const loai_cau = getMainCauName(result.allPatterns);

    setTimeout(() => {
      savePredictionToHistory('b52', phien_hien_tai, du_doan, ti_le);
      recordPrediction('b52', phien_hien_tai, du_doan, ti_le, result.factors);
    }, 0);

    // ✅ ĐỊNH DẠNG CHÍNH XÁC NHƯ BẠN YÊU CẦU
    const output = 
`@cskhvilong1 phien_truoc : ${phien_truoc} : xuc_xac 1 : ${x1} : xuc_xac 2 : ${x2} : xuc_xac 3 : ${x3} : tổng ${tong} : ket_qua : ${ket_qua} : phien_hien_tai : ${phien_hien_tai} : du_doan : ${du_doan} : ${ti_le}% : pattern (${pattern_str}) : loại cầu ${loai_cau}`;

    res.send(output);
    console.log('[lc79‑md5] →', output);

  } catch (err) {
    console.error('[lc79‑md5] Lỗi:', err.message);
    res.status(500).send('@cskhvilong1 Lỗi server xử lý dự đoán');
  }
});
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('  GET /lc79‑md5 ‑ Dự đoán định dạng text @cskhvilong1 ✅ MỚI');
